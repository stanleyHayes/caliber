...
- **CAL-083** `[WIP]` · 5 pts — **Candidate visibility & contest.** Domain + use-case landed: new pure-domain `contest` context (Contest entity with Subject{match,report_card} + open→upheld/dismissed lifecycle, validated) and `app/contest.Service` (Raise / ListForCandidate / ListForSubject / Resolve), every state change appended to the audit trail (explainable + human-in-the-loop). In-memory `ContestRepo` + `AuditRepo` adapters (the latter also makes the audit trail queryable in dev) + generated mocks; domain 100% / app + adapters fully tested. Remaining: proto RPCs + gRPC handler + DI wiring to surface it in the demo. *AC:* surfaced as a fairness feature in the demo. *Deps:* CAL-066
